...
#ifndef WIN32
#define USER_CERT_FILE		".redshift/redshift.crt"
#define USER_KEY_FILE		".redshift/redshift.key"
#define ROOT_CERT_FILE		".redshift/root.crt"
#define ROOT_CRL_FILE		".redshiftroot.crl"
#else
/* On Windows, the "home" directory is already Redshift-specific */
#define USER_CERT_FILE		"redshift.crt"
#define USER_KEY_FILE		"redshift.key"
...
#endif /* USE_SSL */
	{
		...
		if (n < 0)
		{
			...
			switch (result_errno)
			{
				...
				default:
				{
					char *socketError = SOCK_STRERROR(result_errno, sebuf, sizeof(sebuf));
					printfPQExpBuffer(&conn->errorMessage,
						libpq_gettext("could not receive data from server: %s:%d\n"),
									  socketError,result_errno);

#ifdef WIN32
					// 10053:
					// The virtual circuit was terminated due to data transmission time-out or other failure. 
					// The application should close the socket as it is no longer usable.
					if(result_errno == WSAECONNABORTED) // 10053
...
initialize_SSL(PGconn *conn)
{
	...
	bool		have_cert;
	EVP_PKEY   *pkey = NULL;
  char        defsslcertbuffer[MAXPGPATH]; /* Default SSL Certificate*/
  ...
  /* Load default root certificate (root.crt bundled with the driver) */
  if (conn->ssldefaultrootcert && strlen(conn->ssldefaultrootcert) > 0)
  {
      strlcpy(defsslcertbuffer, conn->ssldefaultrootcert, sizeof(defsslcertbuffer));
  }
  else
    defsslcertbuffer[0] = '\0';


  if (defsslcertbuffer[0] != '\0')
  {
      rc = stat(defsslcertbuffer, &buf);
      if(rc == 0)
      {
