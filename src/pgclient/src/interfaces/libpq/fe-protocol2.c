			...
			case SETENV_STATE_CLIENT_ENCODING_SEND:
				{
					...
					if (val)
					{
						if (pg_strcasecmp(val, "default") == 0)
							snprintf(setQuery, sizeof(setQuery), "SET client_encoding = DEFAULT");
						else
							snprintf(setQuery, sizeof(setQuery), "SET client_encoding = '%.60s'",
									val);
#ifdef CONNECTDEBUG
						fprintf(stderr,
			...
			case SETENV_STATE_OPTION_SEND:
				{
					...
					if (conn->next_eo->envName)
					{
						...
						{
							if (pg_strcasecmp(val, "default") == 0)
								snprintf(setQuery, sizeof(setQuery), "SET %s = DEFAULT",
										conn->next_eo->pgName);
							else
								snprintf(setQuery, sizeof(setQuery), "SET %s = '%.60s'",
