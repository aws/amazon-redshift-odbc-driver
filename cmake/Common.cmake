  ...
  if(RS_ODBC_DIR_LIST)
    foreach(child ${RS_ODBC_DIR_LIST})
      if(IS_DIRECTORY ${child}/include)
        target_include_directories(${TARGET_NAME} PRIVATE ${child}/include)
      endif()
      if(IS_DIRECTORY ${child}/lib)
        target_link_directories(${TARGET_NAME} PRIVATE ${child}/lib)
      elseif(IS_DIRECTORY ${child}/lib64)
        target_link_directories(${TARGET_NAME} PRIVATE ${child}/lib64)
      endif()
    endforeach()
  else()
...
function(include_and_link_odbc_test TARGET_NAME)
  message(STATUS "include_and_link_odbc_test for ${TARGET_NAME}")
  message(STATUS "  CMAKE_INCLUDE_PATH: ${CMAKE_INCLUDE_PATH}")
  message(STATUS "  CMAKE_LIBRARY_PATH: ${CMAKE_LIBRARY_PATH}")
  
  # Add non-ODBC dependencies from CMAKE paths (never contains DM)
  target_include_directories(${TARGET_NAME} PRIVATE ${CMAKE_INCLUDE_PATH})
  target_link_directories(${TARGET_NAME} PRIVATE ${CMAKE_LIBRARY_PATH})
  
  # Add DM paths from RS_ODBC_DIR_TEST_LIST
  if(RS_ODBC_DIR_TEST_LIST)
    message(STATUS "  RS_ODBC_DIR_TEST_LIST: ${RS_ODBC_DIR_TEST_LIST}")
    foreach(child ${RS_ODBC_DIR_TEST_LIST})
      if(IS_DIRECTORY ${child}/include)
        message(STATUS "  Adding test DM include: ${child}/include")
        target_include_directories(${TARGET_NAME} PRIVATE ${child}/include)
      endif()
      if(IS_DIRECTORY ${child}/lib)
        message(STATUS "  Adding test DM lib: ${child}/lib")
        target_link_directories(${TARGET_NAME} PRIVATE ${child}/lib)
      elseif(IS_DIRECTORY ${child}/lib64)
        message(STATUS "  Adding test DM lib64: ${child}/lib64")
        target_link_directories(${TARGET_NAME} PRIVATE ${child}/lib64)
      endif()
    endforeach()
  else()
    message(STATUS "  RS_ODBC_DIR_TEST_LIST not set, falling back to RS_ODBC_DIR")
    # Fallback to RS_ODBC_DIR if RS_ODBC_DIR_TEST not set
    include_and_link_odbc(${TARGET_NAME})
  endif()
endfunction()
