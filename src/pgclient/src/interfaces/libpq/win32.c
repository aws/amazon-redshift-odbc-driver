...
static int
LookupWSErrorMessage(DWORD err, char *dest, size_t buflen)
{
	...
	for (e = WSErrors; e->description; e++)
	{
		if (e->error == err)
		{
			strncpy(dest, e->description, buflen);
			return 1;
		}
	}
	return 0;
...
winsock_strerror(int err, char *strerrbuf, size_t buflen)
{
	...
	if (!success)
		snprintf(strerrbuf, buflen, libpq_gettext("Unknown socket error (0x%08X/%i)"), err, err);
	else
	{
		...
		if (offs > (int) buflen - 64)
			offs = buflen - 64;
		snprintf(strerrbuf + offs, buflen - offs, " (0x%08X/%i)", err, err);
	}
	return strerrbuf;
}
