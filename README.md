...
<u>For building on Linux and macOSX:</u>
1. Build above mentioned platform specific dependencies and keep their pre-built binaries in a specific directory.
2. Export `RS_MULTI_DEPS_DIRS` or `RS_DEPS_DIRS` to dependencies directory. You may alternatively set them in`exports_basic.sh` or use  --dependencies-install-dir in the build script. For more details, refer `BUILD.CMAKE.md`.
3. Cd to cloned `amazon-redshift-odbc-driver` package home and build the driver using following command:
...
* macOSX: `/opt/amazon/redshiftodbcx64/`
