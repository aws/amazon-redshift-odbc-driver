...
pg_SSPI_startup(PGconn *conn, int use_negotiate)
{
	...
	len = strlen(conn->krbsrvname) + strlen(conn->pghost) + 2;
	conn->sspitarget = malloc(len);
	if (!conn->sspitarget)
	{
		printfPQExpBuffer(&conn->errorMessage, libpq_gettext("out of memory\n"));
		RS_LOG_DEBUG("ODBC-PQ", "pg_SSPI_startup() return error:%s\n", (conn->errorMessage.data != NULL) ? conn->errorMessage.data : "");
	...
	snprintf(conn->sspitarget, len, "%s/%s", conn->krbsrvname, conn->pghost);
...
pg_password_sendauth(PGconn *conn, const char *password, AuthRequest areq)
{
	...
	switch (areq)
	{
		...
		{
			if (pqGetInt(&algorithm, 4, conn))
			{
				return STATUS_ERROR;	
			}

			// [PGODBC-1432] Support SHA2 password hash
			if (algorithm == AUTH_DIGEST_SHA256)
			{
				int serverSaltLen = 0;
				...
				{
					printfPQExpBuffer(&conn->errorMessage,
						libpq_gettext("out of memory when retrieving the server salt\n"));
					return STATUS_ERROR;
				}
				if (pqGetnchar(serverSalt, serverSaltLen, conn))
				{
					return STATUS_ERROR;	
				}

				// Get nonce length
				if (pqGetInt(&serverNonceLen, 4, conn))
				{
					return STATUS_ERROR;	
				}
				...
				{
					printfPQExpBuffer(&conn->errorMessage,
						libpq_gettext("out of memory when retrieving the server nonce\n"));
					return STATUS_ERROR;
				}
				if (pqGetnchar(serverNonce, serverNonceLen, conn))
				{
					return STATUS_ERROR;	
				}
				...
				*/
				...
				ret = RAND_bytes(nonce_to_send, clientNonceLength);
				if (ret != 1)
				{
					printfPQExpBuffer(&conn->errorMessage,
						libpq_gettext("failed to generate the client's nonce: %s\n"),
				...
				SHA256_Update(&finalDigestCtx, nonce_to_send, clientNonceLength);
				SHA256_Final(finalDigest, &finalDigestCtx);

				free(serverSalt);
				free(serverNonce);

				pwd_to_send = finalDigest;

				break;
			}
			else if (algorithm == AUTH_DIGEST_SCRYPT)
			{
				printfPQExpBuffer(&conn->errorMessage,
					libpq_gettext("Scrypt authentication not supported\n"));
				return STATUS_ERROR;
			}
			else if (algorithm == AUTH_DIGEST_ARGON2)
			{
				printfPQExpBuffer(&conn->errorMessage,
					libpq_gettext("Argon2 authentication not supported\n"));
				return STATUS_ERROR;
			}
		}
	...
	if (areq == AUTH_REQ_DIGEST)
	{
		// Compose a response message and send to the server
		ret = pqDigestResponsePacketSend(
			conn,
			'd',
			pwd_to_send,
...
pg_fe_sendauth(AuthRequest areq, PGconn *conn)
{
	switch (areq)
	{
		...
		case AUTH_REQ_SCM_CREDS:
			...
			case AUTH_REQ_IDP:
			{
				char	   *idp_token = conn->idp_token;

				if (idp_token == NULL || idp_token[0] == '\0')
				{
					printfPQExpBuffer(&conn->errorMessage, PQnoIdpTokenSupplied);
					return STATUS_ERROR;
				}

				if (pqPacketSend(conn, 'i', idp_token, strlen(idp_token) + 1) != STATUS_OK)
				{
					printfPQExpBuffer(&conn->errorMessage,
						"fe_sendauth: error sending AzureAD authentication\n");
					return STATUS_ERROR;
				}
