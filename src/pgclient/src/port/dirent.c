...
opendir(const char *dirname)
{
	...
	strncpy(d->dirname, dirname, strlen(dirname) + 1);
	if (d->dirname[strlen(d->dirname) - 1] != '/' &&
		d->dirname[strlen(d->dirname) - 1] != '\\')
		strncat(d->dirname, "\\",2);		/* Append backslash if not already
										 * there */
	strncat(d->dirname, "*",2);	/* Search for entries named anything */
...
readdir(DIR *d)
{
	...
	strncpy(d->ret.d_name, fd.cFileName, sizeof(d->ret.d_name));		/* Both strings are MAX_PATH
												 * long */
